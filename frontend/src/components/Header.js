  ...
  return (
    <header className='flex flex-row justify-between md:px-5 bg-black text-white py-5'>
        ...
        <div className=' font-bold text-xl md:ml-[500px] '>
            <button onClick={()=>setLogin(!login)} className='mt-8  bg-green-600 md:h-[45px] md:w-[85px]  rounded-md hover:bg-blue-600 hover:scale-110 cursor-pointer hover:duration-500'>ADMIN</button>
        </div>
        ...
        <button className='block md:hidden  font-bold text-4xl mb-3' onClick={()=>setMenu(!menu)}><IoMenu/></button>
         {menu && <nav className=' block md:hidden fixed top-[140px] left-0 bg-gray-300 pt-3 w-full head'>
